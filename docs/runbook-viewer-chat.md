...
Hellō-signed-in viewers with `chat=yes` in `config/users.csv` get an **Ask** pane (Persona). The backend agent is Eino OpenAI **Responses** (`gpt-5-mini` by default) calling the same itinerary ops as MCP, in-process.

## Enable

1. **ACL:** `config/users.csv` column `chat` = `yes` / `true` / `1` (see `users.example.csv`). Rows without `chat` can still sign in.
2. **OpenAI:** set `OPENAI_API_KEY` or `OPENAI_SECRET_JSON` (`{"api_key":"sk-..."}`). Optional `OPENAI_MODEL` (default `gpt-5-mini`).
...
| `model_call` | latency, model, response_id, tool_call count (or error) |
